...
from lfas.models import CrisisType, ProtectionLevel
...
class TestCrisisDetector:
    ...
    def test_assess_crisis_abuse(self):
        ...
        result = vulnerability.detect(
            "partner hurts me, domestic violence, lost my job, can't take it anymore, only chance"
        )
        crisis_result = crisis.assess_crisis(result)

        assert crisis_result.protection_level == ProtectionLevel.CRISIS
        # Should detect abuse in crisis keywords (or mixed/financial due to indicators)
        assert crisis_result.crisis_type in [
            CrisisType.ABUSE,
            CrisisType.MIXED,
    ...
    def test_mixed_crisis_without_mental_health(self):
        ...
        result = vulnerability.detect("lost my job, can't see a doctor, pain won't stop")
        crisis_result = crisis.assess_crisis(result)
    ...
    def test_detected_indicators_extracted(self):
        ...
        result = vulnerability.detect(
            "thinking about suicide, want to die, only chance, last hope, can't take it anymore"
        )
        crisis_result = crisis.assess_crisis(result)
    ...
    def test_user_message_appropriate_for_crisis_type(self):
        ...
        crisis_result = crisis.assess_crisis(result)
        assert (
            "difficult time" in crisis_result.user_message.lower()
            or "safety" in crisis_result.user_message.lower()
        )
